...
//! Phase 12b Part 3c (次 session): KDA forward capture wire-up (`kimi_k3_kda_head_forward` に capture 引数 + `kimi_k3_kda_layer_forward` per-head capture + `KimiK3Model::forward_capture_updates`)、`forward_with_snapshot` の delta mode branch で forward_capture_updates 呼出、Part 3c 完了で **6-10× 実メモリ圧縮 (290MB → 30-48MB)** が実現
//!

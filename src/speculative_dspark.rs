...
//! Phase 12b Part 3c2 (次 session): K3Model 統合 + delta mode wiring: `KimiK3Model::forward_capture_updates(token_id) -> (Vec<f32>, Vec<Vec<KimiDeltaHeadUpdate>>)` (K3 forward 3rd duplicate) + `forward_with_snapshot` delta mode branch (現 delta bound 超過で rebase、そうでなければ forward_capture_updates 呼出 + updates append)、Part 3c2 完了で **6-10× 実メモリ圧縮 (290MB → 30-48MB)** が実現
//!

...
fn main() {
    ...
    println!(
        "Config: hidden={}, heads={}, kv_heads={}, layers={}, vocab={}",
        config.hidden_dim,
        config.num_heads,
        config.num_kv_heads,
        config.num_layers,
    ...
    println!(
        "  token_embd.weight: dims={:?}, qtype={:?}",
        emb_info.dims, emb_info.qtype
    );
    ...
    let attn_norm_w = gguf
        .tensor_to_f32("blk.0.attn_norm.weight")
        .expect("attn_norm");
    ...
    for h in 0..config.num_heads {
        ...
        apply_rope(
            &mut q_buf[start..start + config.head_dim],
            0,
            config.head_dim,
            config.rope_theta,
        );
    ...
    for h in 0..config.num_kv_heads {
        let start = h * config.head_dim;
        apply_rope(
            &mut k_buf[start..start + config.head_dim],
            0,
            config.head_dim,
            config.rope_theta,
        );
    ...
    let o_info = gguf
        ...
        .expect("o info");
    let o_data = gguf
        .tensor_data("blk.0.attn_output.weight")
        .expect("o data");
    ...
    println!("\n=== Step 7: FFN (Layer 0) ===");
    let ffn_norm_w = gguf
        .tensor_to_f32("blk.0.ffn_norm.weight")
        .expect("ffn_norm");
    ...
    println!(
        "  gate: dims={:?}, qtype={:?}",
        gate_info.dims, gate_info.qtype
    );
    println!("  up:   dims={:?}, qtype={:?}", up_info.dims, up_info.qtype);
    println!(
        "  down: dims={:?}, qtype={:?}",
        down_info.dims, down_info.qtype
    );

    let gate_data = gguf
        .tensor_data("blk.0.ffn_gate.weight")
        .expect("gate data");
    let up_data = gguf.tensor_data("blk.0.ffn_up.weight").expect("up data");
    let down_data = gguf
        .tensor_data("blk.0.ffn_down.weight")
        .expect("down data");
    ...
    for i in 0..10 {
        ...
        println!(
            "    [{i}] ours={:.4}, ref={:.4}, diff={diff:+.4}",
            logits[i], REF_LOGITS[i]
        );
    }
    ...
    let output_norm_w = gguf
        .tensor_to_f32("output_norm.weight")
        .expect("output_norm");
    ...
    println!(
        "  output.weight: dims={:?}, qtype={:?}",
        out_info.dims, out_info.qtype
    );
    ...
    let attn_norm1_w = gguf
        .tensor_to_f32("blk.1.attn_norm.weight")
        .expect("blk1 attn_norm");
    ...
    println!(
        "  Layer 1 Q: qtype={:?}, K: qtype={:?}, V: qtype={:?}",
        q1_info.qtype, k1_info.qtype, v1_info.qtype
    );
    ...
    // FFN layer 1
    let ffn_norm1_w = gguf
        .tensor_to_f32("blk.1.ffn_norm.weight")
        .expect("ffn1_norm");
    ...
    for layer_idx in 0..config.num_layers {
        ...
        let an_w = gguf
            .tensor_to_f32(&format!("{prefix}.attn_norm.weight"))
            .unwrap();
        ...
        let oi = gguf
            ...
            .unwrap();
        let qd = gguf
            .tensor_data(&format!("{prefix}.attn_q.weight"))
            .unwrap();
        let kd = gguf
            .tensor_data(&format!("{prefix}.attn_k.weight"))
            .unwrap();
        let vd = gguf
            .tensor_data(&format!("{prefix}.attn_v.weight"))
            .unwrap();
        let od = gguf
            .tensor_data(&format!("{prefix}.attn_output.weight"))
            .unwrap();
        ...
        let di = gguf
            ...
            .unwrap();
        let gd = gguf
            .tensor_data(&format!("{prefix}.ffn_gate.weight"))
            .unwrap();
        let ud = gguf
            .tensor_data(&format!("{prefix}.ffn_up.weight"))
            .unwrap();
        let dd = gguf
            .tensor_data(&format!("{prefix}.ffn_down.weight"))
            .unwrap();

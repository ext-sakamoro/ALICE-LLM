...
fn main() {
    ...
    let dim1 = if emb_info.dims.len() > 1 {
        emb_info.dims[1] as usize
    } else {
        1
    };
    ...
    println!(
        "  token 0 (if row-major, stride=dim0): {:?}",
        &emb[0..8.min(emb.len())]
    );
    println!(
        "  token 1 (if row-major, stride=dim0): {:?}",
        &emb[dim0..dim0 + 8.min(emb.len())]
    );

    // Check if embedding looks reasonable (not all zeros, not all same)
    let sum: f32 = emb[0..dim0.min(emb.len())].iter().sum();
    let max = emb[0..dim0.min(emb.len())]
        .iter()
        .cloned()
        .fold(f32::NEG_INFINITY, f32::max);
    let min = emb[0..dim0.min(emb.len())]
        .iter()
        .cloned()
    ...
    println!(
        "  argmax decode: '{}'",
        tokenizer.decode(&[argmax.0 as u32])
    );

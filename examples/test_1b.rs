use alice_llm::gguf::{dequantize_weight_row, GgufFile};
use std::fs;
...
fn main() {
    let data =
        fs::read("/Users/ys/models/llama-3.2-1b-gguf/Llama-3.2-1B-Instruct-Q4_K_M.gguf").unwrap();
    ...
    println!(
        "\nModel {num_layers}-layer QUANTIZED logits[0:5]: {:?}",
        &model_3l_logits[..5]
    );
    ...
    for i in 0..10 {
        println!(
            "  [{i}] f32={:.6} quant={:.6} diff={:.6}",
            logits[i],
            model_3l_logits[i],
            logits[i] - model_3l_logits[i]
        );
    }

    // Max absolute diff
    let max_diff: f32 = logits
        .iter()
        .zip(&model_3l_logits)
        .map(|(a, b)| (a - b).abs())
        .fold(0.0f32, f32::max);
    let mean_diff: f32 = logits
        .iter()
        .zip(&model_3l_logits)
        .map(|(a, b)| (a - b).abs())
        .sum::<f32>()
    ...
    println!(
        "\nModel {orig_layers}-layer FULL logits[0:5]: {:?}",
        &model_full_logits[..5]
    );
    ...
    println!(
        "  BOS duplicate? generate() prepends bos_id={}, and encode produces [0]={}",
        tok.bos_id, encoded[0]
    );
    ...
    println!(
        "Without <|begin_of_text|> (single BOS): \"{}\"",
        result2.text
    );
    println!(
        "  {} tokens, {:.1} tok/s",
        result2.tokens_generated, result2.tokens_per_sec
    );
}
